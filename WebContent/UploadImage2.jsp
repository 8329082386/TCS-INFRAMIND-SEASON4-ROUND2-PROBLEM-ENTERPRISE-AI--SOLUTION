	...
	if ((contentType.indexOf("multipart/form-data") >= 0)) {
		...
		try {
			...
			try{
				...
				Connection con= DriverManager.getConnection("jdbc:mysql://localhost/imagefusion","root","root");

					//String keyword=request.getParameter("keyword");
					//String keyworddesc=request.getParameter("keyword_desc");
					PreparedStatement prst=con.prepareStatement("insert into imagedata(ImageKeywords,ImagePath)values(?,?)");
					prst.setString(1, "Hello");
					prst.setString(2,fileName);
					prst.executeUpdate();
                 */
				//response.sendRedirect("SplitterImages.jsp?filename="+fileName);
               response.sendRedirect("UpdateImagepath.jsp?filename="+fileName);
              
				 HttpSession hs=request.getSession();
				 
				 hs.setAttribute("target",""+fileName);
				}catch(Exception e){
					out.println(""+e);
				}
